...
class Message extends StatelessWidget {
  ...
  const Message({Key? key, this.currentUserId}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    // StreamBuilder를 사용하여 Firestore의 실시간 데이터를 빌드
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      // 'chat' 컬렉션에서 타임스탬프를 기준으로 내림차순 정렬된 스냅샷 스트림을 가져옴
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (ctx, index) {
            ...
            return Align(
              alignment:
                  isCurrentUser ? Alignment.centerRight : Alignment.centerLeft, // 메시지 정렬 설정
              child: Container(
                padding: EdgeInsets.symmetric(vertical: 8, horizontal: 12), // 내부 패딩 설정
                margin: EdgeInsets.symmetric(vertical: 4, horizontal: 8), // 외부 마진 설정
